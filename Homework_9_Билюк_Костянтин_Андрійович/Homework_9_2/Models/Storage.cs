...
namespace Homework_9_2
{
    class Storage : IComparer, IComparable, IStorage
    {
        ...
        public void QuickSort(int first, int last, int yourPivot)
        {
            if (first < last)
            {
                int left = first;
                int right = last;
                //double middle = Products[(left + right) / 2].Price;
                double pivot = Products[yourPivot].Price;
                do
                {
                    while (Products[left].Price < pivot)
                    {
                        left++;
                    }

                    while (Products[right].Price > pivot)
                    {
                        right--;
                    }
                    if (left <= right)
                    {
                        (Products[left].Price, Products[right].Price) = (Products[right].Price, Products[left].Price);
                        left++;
                        right--;
                    }
                } while (left < right);
                QuickSort(first, right, yourPivot - right);
                QuickSort(left, last, left - yourPivot);
            }
        }

        /// <summary>
        /// Працює з будь-яким опорним елементом в межах масива.
        /// </summary>
        /// <param name="yourPivot"></param>
        /// <exception cref="Exception"></exception>
        public void QuickSort(int yourPivot)
        {
            if (0 > yourPivot || yourPivot >= Products.Count)
                throw new Exception("Pivot went beyond the scope of the array");
            double scaleForPivot = (double)(1 + yourPivot) / (Products.Count);
            QuickSort(0, Products.Count - 1, scaleForPivot);
        }

        private void QuickSort(int first, int last, double scaleForPivot)
        {
            if (first < last)
            {
                int left = first;
                int right = last;
                double pivot = Products[Convert.ToInt32(Math.Ceiling((right - left + 1) * scaleForPivot)) + left - 1].Price;
                do
                {
                    while (Products[left].Price < pivot)
                    {
                ...
                QuickSort(first, right, scaleForPivot);
                QuickSort(left, last, scaleForPivot);
            }
        }
